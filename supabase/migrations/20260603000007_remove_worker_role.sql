-- Remove worker_role column from profiles and drop the enum

-- First update the trigger to stop referencing the `role` column
...
-- Drop the enum (no other table references it)
drop type if exists public.worker_role;
